...
**Algorithm:**
- Generates time slots within each date/time range
- Time slots are spaced 8-15 minutes apart (randomized for realism)
- Distributes readings evenly across all available time slots
- If more readings than slots: cycles through slots
- If fewer readings than slots: spreads them evenly

**Example:**
```
Date Range: 18/03/2026, 09:43 - 12:31 (2h 48m)
Readings: 26 members with varying reading counts
Result: Each reading gets a unique timestamp within the specified period
```
...
**distributeTimestampsAcrossRanges(readingIds, dateTimeRanges)**
- Takes reading IDs and date/time ranges
- Returns array of { readingId, timestamp } pairs
- Handles edge cases (no ranges, more/fewer readings than slots)

**generateTimeSlotsForRange(range)**
- Generates time slots for a single date range
- Uses 8-15 minute intervals with randomization
...
1. **Realistic Reports**: Timestamps show actual inspection workflow
2. **Compliance**: Meets client requirements for detailed time tracking
3. **Flexibility**: Supports multiple date ranges for multi-day inspections
4. **Automatic**: Readings distributed evenly with realistic intervals
5. **Database Persistence**: Timestamps saved permanently for record-keeping

## Limitations & Considerations

1. **Permanent Updates**: Timestamps are updated in the database, not just in the report
2. **Sequential Distribution**: Readings distributed in sequence number order
3. **Time Slots**: Fixed 8-15 minute intervals (configurable in code)
4. **No Validation**: Doesn't prevent overlapping times or past dates
5. **Per-Project**: Each report generation updates all selected readings
