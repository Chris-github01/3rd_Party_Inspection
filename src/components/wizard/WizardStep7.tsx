      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-slate-900">{profile?.name || 'Current User'}</p>
          </div>

          <div>
            <p className="text-sm font-medium text-slate-700">Site Type</p>
            <p className="text-slate-900">{data.siteType}</p>
          </div>

          <div>
            <p className="text-sm font-medium text-slate-700">Project Name</p>
            <p className="text-slate-900 font-semibold">{data.projectName}</p>
          </div>

          {data.package && (
            <div>
              <p className="text-sm font-medium text-slate-700">Package</p>
              <p className="text-slate-900">{data.package}</p>
            </div>
          )}

          <div>
            <p className="text-sm font-medium text-slate-700">Client</p>
            <p className="text-slate-900">{data.clientName}</p>
          </div>

          <div>
            <p className="text-sm font-medium text-slate-700">Setup Mode</p>
            <p className="text-slate-900">{getSetupModeLabel()}</p>
          </div>

          {data.setupMode === 'template' || data.setupMode === 'hybrid' ? (
            <div>
              <p className="text-sm font-medium text-slate-700">Template</p>
              <p className="text-slate-900">{data.templateName}</p>
            </div>
          ) : null}
          ...
          {data.setupMode === 'duplicate' || data.setupMode === 'hybrid' ? (
            <div>
              ...
              <p className="text-slate-900">{data.sourceProjectName}</p>
            </div>
          ) : null}

          <div>
            <p className="text-sm font-medium text-slate-700">Drawing Mode</p>
        ...
        {data.latitude && data.longitude && (
          <div>
            <p className="text-sm font-medium text-slate-700 mb-2">Coordinates</p>
        ...
        {data.what3words && (
          <div>
            <p className="text-sm font-medium text-slate-700 mb-2">what3words</p>
            <p className="text-slate-900 font-mono text-sm">{data.what3words}</p>
          </div>
        )}
