/*
  # Fix coating_system UUID Cast Error in Report Generation Functions

  ## Problem Identified
  - Reports showing "P&R Consulting Limited" instead of project's organization
  - Organization logos not appearing in reports
  - Database functions get_introduction_data() and get_executive_summary_data() CRASH with error:
    "invalid input syntax for type uuid: NULLIFIRE SC902"
  - When functions crash, frontend code falls back to hardcoded "P&R Consulting Limited" default

  ## Root Cause
  The `members.coating_system` field is TEXT containing product names like "NULLIFIRE SC902"
  BUT the report functions try to cast it as UUID: `fpm.id = m.coating_system::uuid`
  This causes query failure → function returns error → frontend uses fallback defaults

  ## Solution
  Fix the JOIN logic to properly handle TEXT coating_system field:
  - Check if coating_system is a valid UUID format first
  - Use proper TEXT comparison when joining to fire_protection_materials table
  - Add error handling to prevent function crashes

  ## Impact
  - Fixes organization name display in all reports
  - Fixes organization logo display in all reports  
  - Ensures data from project.organization_id → organizations table flows correctly to reports
  - Prevents fallback to hardcoded "P&R Consulting Limited" defaults
*/

-- Drop and recreate get_introduction_data with fixed coating_system handling
...
BEGIN
  ...
  -- Detect material types from members (FIXED: Handle TEXT coating_system properly)
  -- Join by product_name match instead of invalid UUID cast
  SELECT
...
-- Drop and recreate get_executive_summary_data with fixed coating_system handling
...
BEGIN
  ...
  -- Get materials from members (FIXED: Handle TEXT coating_system properly)
  -- Join by product_name match instead of invalid UUID cast
  SELECT COALESCE(
...
-- Update comments
COMMENT ON FUNCTION get_introduction_data(uuid) IS 'Returns comprehensive project introduction data including organization info from project.organization_id with proper TEXT coating_system handling';
COMMENT ON FUNCTION get_executive_summary_data(uuid) IS 'Returns executive summary data including organization info from project.organization_id with proper TEXT coating_system handling';
