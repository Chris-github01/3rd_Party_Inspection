/*
  # Fix Report RPCs to Use Project Organization

  1. Problem
    - Reports showing "P&R Consulting Limited" instead of project's organization
    - "Optimal Fire Limited" not appearing in reports despite being assigned to project
    - Logo from organizations table not being used in reports
    - RPC functions query company_settings (global fallback) instead of project.organization_id

  2. Root Cause
    - get_introduction_data() and get_executive_summary_data() pull from company_settings
    - They ignore project.organization_id → organizations table relationship
    - Multi-tenant architecture not properly implemented in report generation

  3. Solution
    - Update RPCs to JOIN projects → organizations table
    - Use organization name and logo_url from organizations table
    - Keep company_settings as fallback only if organization not found
    - Maintain backward compatibility for projects without organization_id

  4. Data Flow
    Before: company_settings → reports (always "P&R Consulting")
    After:  project → organizations → reports (correct org name & logo)
            Fallback to company_settings only if no organization

  5. Testing
    - Projects with organization_id should show that org's name and logo
    - Legacy projects without organization_id should fall back to company_settings
    - All logo URLs should resolve correctly from storage bucket
*/

-- Update get_introduction_data to use project organization
...
BEGIN
  ...
  -- Priority: project.organization_id → organizations table
  -- Fallback: company_settings (for legacy projects)
  SELECT COALESCE(
...
-- Update get_executive_summary_data to use project organization
...
BEGIN
  ...
  -- Build complete result WITH ORGANIZATION DATA
  result := jsonb_build_object(
    'company', company_data,
    'project', project_data,
    'client', COALESCE(client_data, '{}'::jsonb),
...
-- Update comments
COMMENT ON FUNCTION get_introduction_data(uuid) IS 'Returns comprehensive project introduction data including organization info (from project.organization_id → organizations table) with name and logo, scope, dates, blocks/levels, and drawings/pins summary for report generation. Falls back to company_settings if no organization assigned.';
COMMENT ON FUNCTION get_executive_summary_data(uuid) IS 'Returns executive summary data including organization info (from project.organization_id → organizations table) with name and logo, project stats, materials, FRR ratings, inspection statistics, and detailed drawings/pins information for report generation. Falls back to company_settings if no organization assigned.';
