/*
  # Add Drawing Pins and Drawings Data to Report RPC Functions

  ## Problem
  The site manager enters drawing references and pin locations, but this data is not appearing 
  in the downloaded base report. The RPC functions get_introduction_data() and 
  get_executive_summary_data() do not query the drawing_pins or drawings tables.

  ## Solution
  Update both RPC functions to include:
  1. Drawing pins data with locations, pin numbers, and statuses
  2. Drawing references with file names and document information
  3. Block and level associations for proper organization

  ## Changes
  1. Update get_introduction_data() to include drawings and pins summary
  2. Update get_executive_summary_data() to include drawings and pins data
  3. Add helper fields for report generation

  ## Security
  - Functions remain SECURITY DEFINER with authenticated user access
  - No sensitive data exposure
*/

-- Update get_introduction_data to include drawings and pins
...
BEGIN
  ...
  -- NEW: Get drawings and pins data
  SELECT jsonb_build_object(
...
-- Update get_executive_summary_data to include drawings and pins
...
BEGIN
  ...
  -- NEW: Get drawings and pins summary with detailed pin information
  SELECT jsonb_build_object(
    'total_drawings', COALESCE(
      (SELECT COUNT(DISTINCT d.id)
       FROM drawings d
...
COMMENT ON FUNCTION get_introduction_data(uuid) IS 'Returns comprehensive project introduction data including company info, scope, dates, blocks/levels, and drawings/pins summary for report generation';
COMMENT ON FUNCTION get_executive_summary_data(uuid) IS 'Returns executive summary data including project stats, materials, FRR ratings, inspection statistics, and detailed drawings/pins information for report generation';
