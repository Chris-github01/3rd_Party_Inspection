/*
  # Fix Drawing Pins SQL Syntax Error in RPC Functions

  ## Problem
  The get_executive_summary_data() and get_introduction_data() functions have SQL syntax errors
  causing them to fail when querying drawing pins data. The ORDER BY clause inside jsonb_agg()
  requires explicit sorting in the subquery.

  ## Solution
  Remove ORDER BY from inside jsonb_agg() or move it to a proper position. The pins will be
  returned in creation order, and sorting can be done on the client side if needed.

  ## Changes
  - Fix get_executive_summary_data() drawing pins query
  - Fix get_introduction_data() drawings list query
*/

-- Drop and recreate get_executive_summary_data with fixed syntax
...
BEGIN
  ...
  -- FIXED: Get drawings and pins summary with proper aggregation
  SELECT jsonb_build_object(
    ...
    'pins_summary', COALESCE(
      (SELECT jsonb_agg(pin_data)
       FROM (
         ...
         WHERE dp.project_id = p_project_id
         ORDER BY dp.pin_number NULLS LAST, dp.created_at
       ) pins),
      '[]'::jsonb
    )
  )
...
-- Drop and recreate get_introduction_data with fixed syntax
...
BEGIN
  ...
  -- FIXED: Get drawings and pins data with proper subquery
  SELECT jsonb_build_object(
    'total_drawings', COALESCE(
      (SELECT COUNT(DISTINCT d.id)
       FROM drawings d
    ...
    'drawings_list', COALESCE(
      (SELECT jsonb_agg(drawing_data)
       FROM (
         ...
         WHERE b.project_id = p_project_id
         ORDER BY b.name, l.name, d.page_number
       ) drawings_ordered),
      '[]'::jsonb
    )
  )
